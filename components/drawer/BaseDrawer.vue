...
<template>
  <n-drawer v-model:show="active" :width="200" :placement="placement" class="bg-black ">
    <n-drawer-content @click="close()">
      <NuxtLink v-for="(item,index) in navigation" :key="index" :class="[item.current ? 'text-yellow-500' : 'text-white  hover:text-yellow-500', 'block px-3 py-2 rounded-md text-base font-medium']" :to="item.href" @click="switchNav(index)">
        {{ item.name }}
      </NuxtLink>
    </n-drawer-content>
  </n-drawer>
